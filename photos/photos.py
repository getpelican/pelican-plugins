...
def watermark_photo(image, watermark_text, watermark_image, watermark_image_size):
    ...
    if watermark_image:
        ...
        mark_image_size = watermark_image_size if watermark_image_size else mark_image_size
...
def resize_worker(orig, resized, spec, settings):
    ...
    if settings['PHOTO_WATERMARK']:
        im = watermark_photo(im, settings['PHOTO_WATERMARK_TEXT'], settings['PHOTO_WATERMARK_IMG'], settings['PHOTO_WATERMARK_IMG_SIZE'])
...
def resize_photos(generator, writer):
    ...
    logger.debug('Debug Status: {}'.format(DEFAULT_CONFIG['debug']))
