...
After typing "make html" when using the `ipynb` tag, three files are
...
within the blog theme, but there may still be some conflicts. You might want to
override some of the `ipynb.css`-styles, which you can put into
...
#  IPython/nbconvert/templates/fullhtml.tpl.
#  Put personal adjustements into output/theme/css/ipynb_mod.py.
CSSJS_HEADER = r"""
/*
...
def notebook(preprocessor, tag, markup):
    ...
    # if we haven't already saved the css/js-files for ipynb, save it here.
    if not notebook.header_saved:
        print ("\n ** Writing styles to output/theme/css/ipynb.css,"
               "output/theme/js/collapse.js, and output/theme/js/math.js. **\n")

        header = '\n'.join(CSS_WRAPPER.format(css_line)
                           for css_line in resources['inlining']['css'])

        if not os.path.exists('output/theme/css'):
            os.makedirs('output/theme/css')
        if not os.path.exists('output/theme/js'):
            os.makedirs('output/theme/js')
        with open('output/theme/css/ipynb.css', 'w') as f:
            f.write(header)
        with open('output/theme/js/math.js', 'w') as f:
            f.write(JS_MATH)
        with open('output/theme/js/collapse.js', 'w') as f:
            f.write(JS_COLLAPSE)
