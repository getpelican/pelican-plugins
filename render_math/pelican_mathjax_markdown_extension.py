...
class PelicanMathJaxPattern(markdown.inlinepatterns.Pattern):
    ...
    def __init__(self, pelican_mathjax_extension, tag, pattern):
        ...
        self.math_tag_class = pelican_mathjax_extension.getConfig(
            'math_tag_class')
    ...
    def handleMatch(self, m):
        ...
        # If mathjax was successfully matched,
        # then JavaScript needs to be added for rendering.
        self.pelican_mathjax_extension.mathjax_needed = True
        return node


class PelicanMathJaxCorrectDisplayMath(Treeprocessor):
    """
    ...
    def run(self, root):
        ...
        for parent in root:
            ...
            self.correct_html(
                root, children, div_math, insert_idx, parent.text
            )
...
class PelicanMathJaxExtension(markdown.Extension):
    ...
    def __init__(self, config):
        try:
            ...
            self.config['auto_insert'] = [
                True,
                'Determines if mathjax script is automatically inserted'
                'into content']
            ...
            config['auto_insert'] = [
                config['auto_insert'],
                'Determines if mathjax script is '
                'automatically inserted into content']
            super(PelicanMathJaxExtension, self).__init__(config)
    ...
    def extendMarkdown(self, md, md_globals):
        ...
        # Process mathjax before escapes are processed
        ...
        md.inlinePatterns.add(
            ...
            PelicanMathJaxPattern(
                self, 'div', mathjax_display_regex
            ),
            '<escape')
        md.inlinePatterns.add(
            'mathjax_inlined',
            PelicanMathJaxPattern(
                self, 'span', mathjax_inline_regex
            ),
            '<escape')
        ...
        # If necessary, add the JavaScript Mathjax library to the document.
